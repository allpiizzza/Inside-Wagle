...
- `8월 프리덤`, `9월 프리덤`, ... — 숫자(Number) 타입. 필요한 달만 추가하면 돼요
- 30명 명단을 미리 다 등록해두세요. **명단에 없는 이름으로 인증/등록하면 실패하도록** 만들어뒀어요.

## 2. 퀘스트 DB 만들기
- `퀘스트` — 제목(Title) 타입
- `현재 진행중` — 체크박스(Checkbox) 타입
- `달성조건` — 텍스트(Text) 타입
- `점수` — 숫자(Number) 타입

## 3. 기억구슬 DB 만들기
- `책 제목` — 제목(Title) 타입 (기본 Name 속성 이름을 "책 제목"으로 변경해도 됨)
...
├── freedom.html           # 프리덤 독서 기록 (날짜+시간 선택 → 월별 +1)
└── api/
    ...
    └── freedom.js           # 날짜의 월에 맞는 "N월 프리덤" 값 +1
```
